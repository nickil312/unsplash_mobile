...
import {fetchBannedPosts, fetchReportedPosts} from "@/globalRedux/posts/asyncActions";
import {countview, license, orientation, sort} from "@/app/(tabs)/(home)";
...
import ReportCard from "@/components/miniPost/ReportCard";
...
export default function Reported(){
    ...
    const {status} = useSelector((state: RootState) => state.posts.ReportPosts);
    const {api_url} = useSelector((state: RootState) => state.posts);
    const router = useRouter();
    const [page, setPage] = useState<number>(0);
    const [posts, setPosts] = useState<ReportsCardProps[]>([]);
        ...
        >
            ...
            {refreshing && status === Status.LOADING ? ( // Проверяем состояние загрузки
                ...
                <VirtualizedList
                    ...
                    getItem={getItem}
                    renderItem={({item}: { item: ReportsCardProps }) => (
                        <TouchableOpacity onPress={() => {
                            router.push(`/reported/${item._id}`)
                        }}>
                            <ReportCard api_url={api_url} _id={item._id} text={item.text} imageurl={item.imageurl} post_id={item.post_id} user_id={item.user_id} title={item.title} report_count={item.report_count}  fullname={item.fullname} banned={item.banned} />
                        </TouchableOpacity>
                    )}
